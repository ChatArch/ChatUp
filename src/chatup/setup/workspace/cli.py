...
from chatup.utils.pathing import display_path, resolve_workspace_dir, write_text_file
...
def setup_workspace(
    ...
    usage = (
        "Usage: chatup workspace [PROFILE] [WORKSPACE_DIR] "
    ...
    for item in applied:
        ...
        if item["name"] == "chatblog":
            ...
            if item.get("public_source_label"):
                click.echo(f"Public link source: {item['public_source_label']}")
        if item["name"] == "memory":
